car_data = {
  ...
  'Nissan Titan': ('silver', 2016, 5.6, 'pickup', 35000)
}
search_criteria = (2017, 1.6, 36000)

# Критерії пошуку
year_exact, engine_exact, price_exact = search_criteria

# Спроба суворої відповідності
strict_filtered_cars = [
    (name, data) for name, data in car_data.items()
    if data[1] == year_exact and data[2] == engine_exact and data[4] == price_exact
]

# Якщо сувора відповідність не дала результату, послаблюємо фільтр
if not strict_filtered_cars:
    relaxed_filtered_cars = [
        (name, data) for name, data in car_data.items()
        if data[1] >= year_exact and data[2] >= engine_exact and data[4] <= price_exact
    ]
else:
    relaxed_filtered_cars = strict_filtered_cars  # Використовуємо суворий результат, якщо він є

# Сортуємо за ціною
sorted_cars = sorted(relaxed_filtered_cars, key=lambda item: item[1][-1])

# Виводимо до 5 перших знайдених автомобілів
